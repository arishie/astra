...
router.post('/ingest', rateLimit('ingest', 1), async (req, res) => {
    ...
    try {
        ...
        for (const chunk of chunks) {
            const chunkId = await manager.addMemory(chunk, {
                ...metadata,
                type,
                documentId: docId,
...
router.get('/search', rateLimit('chat', 1), async (req, res) => {
    ...
    try {
        ...
        const thresholdNum = parseFloat(threshold) || 0.7;
        const results = await manager.search(query, limitNum, req.userId);
        // Filter by threshold if scores are available
        const filteredResults = results.filter(r => r.score === undefined || r.score >= thresholdNum);
        res.json({
