...
let orchestratorHandler: ((userId: string, message: string, context: any) => Promise<string>) | null = null;

export function setBridgeManager(manager: BridgeManager): void {
    bridgeManager = manager;
}

export function setOrchestratorHandler(
    handler: (userId: string, message: string, context: any) => Promise<string>
): void {
...
router.post('/', rateLimit('chat', 1), async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        if (platform && recipient && bridgeManager) {
            const sent = await bridgeManager.sendMessage(
                req.userId!,
...
router.post('/stream', rateLimit('chat', 1), async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        ...
        if (!orchestratorHandler) {
            res.write(
                `data: ${JSON.stringify({ type: 'error', message: 'Orchestrator not available' })}\n\n`
            );
            res.end();
            return;
        }
...
router.get('/history', async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        ...
        const params: any[] = [req.userId!, limitNum];
        let query = `
        ...
        if (before) {
            ...
            params.push(new Date(before as string));
        }
...
router.post('/hive', rateLimit('hive', 1), async (req: AuthenticatedRequest, res: Response) => {
