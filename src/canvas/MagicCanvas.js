...
export class APINode extends WorkflowNode {
    ...
    async execute(inputs, context) {
        ...
        const outputs = new Map();
        try {
            const url = inputs.get('url') || this.config.url;
            const inputBody = inputs.get('body');
            const inputHeaders = inputs.get('headers');
            if (!url) {
...
export class TransformNode extends WorkflowNode {
    ...
    async execute(inputs, context) {
        ...
        try {
            ...
            const transformType = this.config.transformType;
            let result;
            switch (transformType) {
                case 'jmespath':
                    result = this.applyJMESPath(data, this.config.expression);
                    break;
                case 'mapping':
                    result = this.applyMapping(data, this.config.mapping);
                    break;
                case 'custom':
                    result = this.applyCustomFunction(data, this.config.customFunction);
                    break;
                case 'stringify':
                ...
                case 'reverse':
                    ...
                    break;
                case 'filter':
                    result = this.applyFilter(data, this.config.expression);
                    break;
                case 'map':
                    result = this.applyMap(data, this.config.expression);
                    break;
                case 'reduce':
                    result = this.applyReduce(data, this.config.expression);
                    break;
                default:
                    result = data;
            }
    ...
    applyCustomFunction(data, fnString) {
        if (!fnString)
            return data;
        try {
...
export class InputNode extends WorkflowNode {
    ...
    async execute(inputs, context) {
        ...
        try {
            ...
            const inputKey = this.config.inputKey || this.id;
...
export class OutputNode extends WorkflowNode {
    ...
    async execute(inputs, context) {
        ...
        try {
            const value = inputs.get('value');
            const outputKey = this.config.outputKey;
...
export class SplitNode extends WorkflowNode {
    ...
    async execute(inputs, context) {
        ...
        try {
            const data = inputs.get('data');
            const splitType = this.config.splitType;
            let items;
            switch (splitType) {
                case 'array':
...
export class WebhookNode extends WorkflowNode {
    ...
    async execute(inputs, context) {
        ...
        try {
            ...
            const webhookData = context.variables.get(`webhook_${this.id}`);
            if (webhookData) {
...
export class WorkflowGraph {
    ...
    getInputNodes() {
        return this.getNodes().filter(node => node.type === 'input' || node.type === 'webhook' ||
            (this.reverseAdjacencyList.get(node.id)?.size ?? 0) === 0);
    }
    getOutputNodes() {
        return this.getNodes().filter(node => node.type === 'output' ||
...
export class ValidationEngine {
    ...
    validateGraphStructure(graph, errors, warnings) {
    ...
    validateConnections(graph, errors, warnings) {
    ...
    validateDataFlow(graph, errors, warnings) {
...
export class WorkflowExecutor {
    ...
    async execute(graph, inputs = {}, options = {}) {
    ...
    gatherNodeInputs(graph, nodeId, context) {
        const inputs = new Map();
        const connections = graph.getConnectionsToNode(nodeId);
        for (const conn of connections) {
            const sourceOutputs = context.nodeOutputs.get(conn.sourceNodeId);
            if (sourceOutputs) {
...
export class WorkflowCompiler {
    compile(workflow, language = 'typescript') {
    ...
    compileToTypeScript(graph, workflow) {
        const executionOrder = graph.topologicalSort();
        const lines = [
            '/**',
    ...
    compileToPython(graph, workflow) {
        const executionOrder = graph.topologicalSort();
        const lines = [
            '"""',
            `Auto-generated workflow: ${workflow.name}`,
    ...
    generatePythonNodeExecutor(nodeType) {
        const lines = [];
        switch (nodeType) {
            case 'llm':
...
export class NaturalLanguageBuilder {
    ...
    registerDefaultPatterns() {
        ...
        this.patterns.set(/call\s+(?:an?\s+)?api\s+(?:at\s+)?(?:endpoint\s+)?['""]?([^\s'"]+)['""]?/i, (match) => ({
            nodes: [{
                    type: 'api',
        ...
        this.patterns.set(/(?:make\s+(?:an?\s+)?llm\s+call|ask\s+(?:the\s+)?ai|generate\s+(?:text|response)|use\s+(?:gpt|claude|llm))/i, () => ({
            nodes: [{
                    type: 'llm',
                    name: 'AI Response',
        ...
        this.patterns.set(/if\s+(.+?)\s+(?:then|do)/i, (match) => ({
            nodes: [{
                    type: 'condition',
                    name: `Check: ${match[1]}`,
        ...
        this.patterns.set(/(?:loop\s+through|for\s+each|iterate\s+(?:over|through))\s+(.+)/i, (match) => ({
            nodes: [{
                    type: 'loop',
                    name: `Loop: ${match[1]}`,
        ...
        this.patterns.set(/(?:transform|convert|map|filter|format)\s+(?:the\s+)?(?:data|result|response)?/i, () => ({
            nodes: [{
                    type: 'transform',
                    name: 'Transform Data',
        ...
        this.patterns.set(/(?:wait|delay|pause)\s+(?:for\s+)?(\d+)\s*(?:ms|milliseconds?|s(?:ec(?:ond)?s?)?|m(?:in(?:ute)?s?)?)?/i, (match) => {
    ...
    extractWorkflowName(description) {
    ...
    registerPattern(pattern, builder) {
...
export class TemplateLibrary {
    ...
    createConditionalRouterWorkflow() {
...
export class MagicCanvas {
     ...
     */
    compileWorkflow(workflow, language = 'typescript') {
     ...
     */
    serializeWorkflow(workflow, name, description = '') {
    ...
    graphToWorkflowData(graph, name, description) {
...
//# sourceMappingURL=MagicCanvas.js.map