...
import type { BridgeMessage, Platform } from '../bridge/BaseBridge.js';
...
export interface MultiTenantConfig {
    ...
    userContextTTL?: number; // ms - how long to keep inactive user contexts
...
export class MultiTenantOrchestrator extends EventEmitter {
    ...
    private sharedMemory: LanceManager; // For system-level data only
     ...
     */
    private async initializeSystemAuth(): Promise<void> {
     ...
     */
    public async getUserContext(userId: string, accessToken?: string): Promise<UserContext | null> {
     ...
     */
    private async createUserContext(userId: string, email: string): Promise<UserContext> {
     ...
     */
    private async loadUserModels(userId: string, modelRouter: ModelRouter): Promise<void> {
        try {
            ...
            for (const keyInfo of apiKeys) {
                ...
                const decryptedKey = await this.userService.getApiKey(
                    userId,
                    keyInfo.provider,
                    keyInfo.name
                );
                ...
                if (decryptedKey) {
                    modelRouter.getRegistry().registerModel({
                        name: keyInfo.name,
                        providerType: keyInfo.provider as any,
                        modelId: keyInfo.modelId || keyInfo.name,
                        apiKey: decryptedKey,
                        baseUrl: keyInfo.baseUrl,
                        tier: (keyInfo.tier as 1 | 2 | 3) || 1,
                    });
                }
            }
        } catch (error) {
     ...
     */
    public async handleUserMessage(userId: string, message: BridgeMessage): Promise<void> {
        // Rate limit check
        ...
        if (!limitResult.allowed) {
            ...
            await this.sendErrorToUser(userId, message.platform, message.sender,
                `Rate limit exceeded. Please wait ${limitResult.retryAfter} seconds.`);
            return;
        }
        ...
        if (!context) {
            await this.sendErrorToUser(userId, message.platform, message.sender,
                'Authentication required. Please log in at https://astra.ai');
            return;
        }

        try {
            const response = await this.processUserMessage(context, message);
            await this.bridgeManager.sendMessage(userId, message.platform as Platform, message.sender, response);
        } catch (error) {
     ...
     */
    private async processUserMessage(context: UserContext, message: BridgeMessage): Promise<string> {
     ...
     */
    private async handleCommand(context: UserContext, content: string, message: BridgeMessage): Promise<string> {
...
Manage your API keys and platforms at https://astra.ai/settings`;
    ...
    private async sendErrorToUser(
        ...
        try {
            await this.bridgeManager.sendMessage(userId, platform as Platform, recipient, `⚠️ ${message}`);
        } catch (error) {
     ...
     */
    private async destroyUserContext(userId: string): Promise<void> {
        const context = this.userContexts.get(userId);
        if (!context) return;
