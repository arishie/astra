...
import {} from '../bridge/BaseBridge.js';
...
export class AstraOrchestrator {
     ...
     */
    linkPlatformUser(platform, platformSenderId, userId) {
     ...
     */
    getLinkedUserId(platform, platformSenderId) {
        const key = `${platform}:${platformSenderId}`;
        return this.authenticatedUsers.get(key) || null;
    }
    getBridgeByPlatform(platform) {
        if (platform === 'whatsapp')
            return this.bridges[0];
        if (platform === 'telegram')
            return this.bridges[1];
        if (platform === 'discord')
            return this.bridges[2];
        if (platform === 'slack')
            return this.bridges[3];
        return undefined;
    }
    async start() {
    ...
    async handleIncomingMessage(msg) {
        ...
        if (!userId) {
            ...
            await bridge.sendMessage(sender, "⚠️ Authentication Required\n\n" +
        ...
        try {
            if (trimmedMsg.startsWith('/hive')) {
                ...
                const config = { name, providerType, modelId, apiKey };
                if (baseUrl)
    ...
    async executeCommandRequest(sender, command, bridge) {
    ...
    async processQuery(sender, query, bridge) {
...
//# sourceMappingURL=AstraOrchestrator.js.map