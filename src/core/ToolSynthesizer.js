...
export class ToolSynthesizer {
    ...
    capabilities;
    sandbox;
    dynamicToolsDir = 'src/dynamic_tools';
    tempDir = 'src/dynamic_tools/temp';
    constructor(modelRouter, capabilities) {
        this.modelRouter = modelRouter;
        this.capabilities = capabilities;
        // Ideally utilize Docker for synthesis testing
        this.sandbox = new SandboxManager(SandboxMode.HOST);
    }
    async synthesize(goal, sessionToken) {
    ...
    async generateCode(goal, lastError) {
        const prompt = `
        GOAL: ${goal}        
