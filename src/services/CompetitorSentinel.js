...
export class WebsiteMonitor {
     ...
     */
    async startMonitoring(competitor, onSnapshot) {
     ...
     */
    async captureSnapshot(competitorId, url) {
        try {
     ...
     */
    async captureWithScreenshotApi(url, apiKey) {
     ...
     */
    async captureWithBrowserless(url, endpoint) {
     ...
     */
    async simulateScreenshotCapture(url) {
     ...
     */
    getLatestSnapshot(competitorId, url) {
...
export class VisualDiffEngine {
     ...
     */
    calculateDiffPercentage(before, after) {
     ...
     */
    async identifyChangedRegions(before, after, diffPercentage) {
     ...
     */
    async generateDiffImage(before, after) {
    ...
    categorizeChange(diff) {
        ...
        const hasPricingRegion = diff.changedRegions.some(r => r.significance > 50 && r.width > 300);
        if (hasPricingRegion) {
            return 'pricing';
        }
        if (hasHeaderChange) {
...
export class PriceTracker {
     ...
     */
    async extractPrices(competitorId, url, htmlContent) {
        const prices = [];
     ...
     */
    extractPricesFromSection(section, competitorId, url) {
        const prices = [];
        ...
        for (const pattern of this.pricePatterns) {
            ...
            while ((match = pattern.exec(textContent)) !== null) {
                ...
                if (!isNaN(numericPrice) && numericPrice > 0 && numericPrice < 100000) {
                    ...
                    prices.push(this.createPriceData(competitorId, url, tiers[tierIndex] || `Plan ${prices.length + 1}`, numericPrice, 'USD', tiers[tierIndex], billingCycle));
                }
            }
        }
        return prices;
     ...
     */
    extractAllPrices(text, competitorId, url) {
        const prices = [];
        let planIndex = 1;
        for (const pattern of this.pricePatterns) {
            let match;
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (!isNaN(numericPrice) && numericPrice > 0 && numericPrice < 100000) {
                    const billingCycle = this.detectBillingCycle(match[0]);
                    prices.push(this.createPriceData(competitorId, url, `Plan ${planIndex++}`, numericPrice, 'USD', undefined, billingCycle));
                }
            }
        }
        return prices;
     ...
     */
    createPriceData(competitorId, url, productName, price, currency, tier, billingCycle) {
        return {
            id: crypto.randomUUID(),
            competitorId,
            productName,
     ...
     */
    detectChanges(competitorId, currentPrices, previousPrices) {
        const changes = [];
        const previousMap = new Map(previousPrices.map(p => [`${p.productName}:${p.tier || ''}`, p]));
        const currentMap = new Map(currentPrices.map(p => [`${p.productName}:${p.tier || ''}`, p]));
...
export class ProductCatalogTracker {
     ...
     */
    async extractProducts(competitorId, url, htmlContent) {
        const products = [];
     ...
     */
    parseProductContainer(container, competitorId, url) {
     ...
     */
    detectChanges(competitorId, currentProducts) {
        const changes = [];
     ...
     */
    detectModifications(existing, current) {
        const modifications = {};
...
export class SocialMediaMonitor {
    ...
    async monitor(competitor) {
        ...
        for (const profile of competitor.socialProfiles) {
            ...
            try {
                ...
                console.error(`[SocialMediaMonitor] Failed to monitor ${profile.platform}:`, error);
            }
        }
     ...
     */
    async fetchPosts(competitorId, profile) {
     ...
     * Fetch Twitter posts.
     */
    async fetchTwitterPosts(competitorId, profile, apiConfig) {
     ...
     */
    async fetchLinkedInPosts(competitorId, profile, apiConfig) {
     ...
     */
    async fetchFacebookPosts(competitorId, profile, apiConfig) {
     ...
     */
    async fetchInstagramPosts(competitorId, profile, apiConfig) {
     ...
     */
    async fetchYouTubePosts(competitorId, profile, apiConfig) {
     ...
     */
    generateMockPosts(competitorId, profile, platform) {
        const posts = [];
        const now = new Date();
        const topics = [
     ...
     */
    async fetchMetrics(competitorId, profile) {
        // In production, fetch actual metrics from APIs
        const posts = this.socialData.get(`${competitorId}:${profile.platform}`) || [];
        const recentPosts = posts.filter(p => p.postedAt > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000));
        ...
        const metrics = {
            ...
            topPosts: posts.sort((a, b) => (b.engagement.likes + b.engagement.shares) - (a.engagement.likes + a.engagement.shares)).slice(0, 3),
            capturedAt: new Date(),
        };
     ...
     */
    getRecentPosts(competitorId, platform) {
        if (platform) {
    ...
    analyzeActivity(competitorId) {
        ...
        // Calculate metrics
        const totalEngagement = allPosts.reduce((sum, p) => sum + p.engagement.likes + p.engagement.comments + p.engagement.shares, 0);
        const avgSentiment = allPosts.reduce((sum, p) => sum + (p.sentiment || 0), 0) / allPosts.length;
        // Find top platform
        ...
        const lastWeek = allPosts.filter(p => p.postedAt > twoWeeksAgo && p.postedAt <= oneWeekAgo).length;
...
export class CompetitorAlertSystem extends EventEmitter {
    ...
    async dispatchAlert(alert) {
        ...
        for (const channelConfig of this.channelConfigs) {
            ...
            try {
                ...
                console.error(`[AlertSystem] Failed to send alert to ${channelConfig.type}:`, error);
            }
        }
    }
    /**
     ...
     */
    async sendToChannel(alert, config) {
        switch (config.type) {
            case 'email':
...
export class ReportGenerator {
    ...
    async generateReport(params) {
        const { competitors, periodStart, periodEnd, priceChanges, productChanges, visualChanges, socialPosts, alerts, reportType = 'weekly', } = params;
        // Generate competitor summaries
        const competitorSummaries = competitors.map(competitor => this.generateCompetitorSummary(competitor, priceChanges, productChanges, visualChanges, alerts));
        // Filter period-relevant data
        const periodPriceChanges = priceChanges.filter(c => c.detectedAt >= periodStart && c.detectedAt <= periodEnd);
        const periodProductChanges = productChanges.filter(c => c.detectedAt >= periodStart && c.detectedAt <= periodEnd);
        const periodVisualChanges = visualChanges.filter(c => c.analyzedAt >= periodStart && c.analyzedAt <= periodEnd);
        // Get social highlights
        const socialHighlights = this.selectSocialHighlights(socialPosts, periodStart, periodEnd);
        // Generate insights
        const keyInsights = this.generateKeyInsights(competitorSummaries, periodPriceChanges, periodProductChanges, socialHighlights);
        // Generate recommendations
        const recommendations = this.generateRecommendations(competitorSummaries, periodPriceChanges, periodProductChanges);
        const report = {
            id: crypto.randomUUID(),
            reportType,
            periodStart,
     ...
     */
    generateCompetitorSummary(competitor, priceChanges, productChanges, visualChanges, alerts) {
        ...
        const changesByType = {
            ...
            feature: 0,
            content: visualChanges.filter(c => c.competitorId === competitorId && c.diffPercentage < 30).length,
            design: visualChanges.filter(c => c.competitorId === competitorId && c.diffPercentage >= 30).length,
            social: 0,
        };
        const totalChanges = Object.values(changesByType).reduce((a, b) => a + b, 0);
        // Identify significant changes
        const significantChanges = [];
        const majorPriceChanges = priceChanges.filter(c => c.competitorId === competitorId && Math.abs(c.changePercentage) > 10);
        for (const change of majorPriceChanges) {
            significantChanges.push(`${change.changeType === 'increase' ? 'Price increase' : 'Price decrease'} ` +
                `for ${change.productName}: ${change.changePercentage > 0 ? '+' : ''}${change.changePercentage.toFixed(1)}%`);
        }
        const newProducts = productChanges.filter(c => c.competitorId === competitorId && c.changeType === 'added');
     ...
     */
    selectSocialHighlights(posts, periodStart, periodEnd) {
        return posts
     ...
     */
    generateKeyInsights(summaries, priceChanges, productChanges, socialHighlights) {
        const insights = [];
        ...
        if (priceIncreases > priceDecreases) {
            insights.push(`Market trend: ${priceIncreases} price increases vs ${priceDecreases} decreases. ` +
                'Consider reviewing your pricing strategy.');
        }
        else if (priceDecreases > priceIncreases) {
            insights.push(`Market trend: ${priceDecreases} price decreases detected. ` +
        ...
        if (newProducts > 0) {
            insights.push(`${newProducts} new products/features launched by competitors. ` +
        ...
        if (highRiskCompetitors.length > 0) {
            insights.push(`High activity detected from: ${highRiskCompetitors.map(c => c.competitorName).join(', ')}. ` +
                'These competitors warrant close attention.');
        }
        // Social engagement insight
        if (socialHighlights.length > 0) {
            const avgEngagement = socialHighlights.reduce((sum, p) => sum + p.engagement.likes + p.engagement.shares, 0) / socialHighlights.length;
            insights.push(`Top competitor social posts averaged ${Math.round(avgEngagement)} engagements. ` +
     ...
     */
    generateRecommendations(summaries, priceChanges, productChanges) {
        const recommendations = [];
        // Pricing recommendations
        const significantPriceChanges = priceChanges.filter(c => Math.abs(c.changePercentage) > 15);
        if (significantPriceChanges.length > 0) {
            recommendations.push('Review pricing strategy: Multiple competitors made significant price adjustments. ' +
        ...
        if (newFeatures.length > 2) {
            recommendations.push('Feature gap analysis recommended: Competitors launched multiple new features. ' +
        ...
        if (highActivityCompetitors.length > 0) {
            recommendations.push(`Increase monitoring frequency for: ${highActivityCompetitors.map(c => c.competitorName).join(', ')}. ` +
                'These competitors are highly active.');
        }
        // Default recommendation
        if (recommendations.length === 0) {
            recommendations.push('Market appears stable. Continue regular monitoring and focus on your strategic initiatives.');
        }
    ...
    async saveReport(report) {
        if (!this.db)
            return;
        try {
            await this.db.query(`INSERT INTO competitor_reports
...
export class ChangeHistory {
    ...
    async recordChange(change) {
        ...
        if (this.db && this.db.isReady()) {
            try {
                await this.db.query(`INSERT INTO competitor_changes
    ...
    async getHistoryFromDb(options) {
        ...
        try {
            const result = await this.db.query(`SELECT * FROM competitor_changes ${whereClause}
     ...
     */
    async getStats(competitorId, days = 30) {
...
export class CompetitorSentinel extends EventEmitter {
     ...
     */
    async startCompetitorMonitoring(competitor) {
     ...
     */
    async handleNewSnapshot(competitor, snapshot) {
    ...
    async checkPricing(competitor) {
        ...
        try {
            ...
            // Extract prices
            const currentPrices = await this.priceTracker.extractPrices(competitor.id, competitor.pricingPageUrl, html);
            // Get previous prices
     ...
     */
    async checkProducts(competitor) {
        if (!competitor.productPageUrl)
            return;
        try {
            ...
            const html = await response.text();
            // Extract products
            const currentProducts = await this.productTracker.extractProducts(competitor.id, competitor.productPageUrl, html);
            // Detect changes
            const changes = this.productTracker.detectChanges(competitor.id, currentProducts);
            for (const change of changes) {
                const severity = change.changeType === 'added' ? 'high' : 'medium';
                // Record change
                await this.changeHistory.recordChange({
                    competitorId: competitor.id,
                    changeType: 'product',
     ...
     */
    async checkSocialMedia(competitor) {
        if (competitor.socialProfiles.length === 0)
            return;
        try {
    ...
    async generateWeeklyReport() {
        ...
        for (const competitor of competitors) {
            ...
            for (const record of history) {
                if (record.metadata) {
                    priceChanges.push(record.metadata);
                }
            }
            // Get social posts
            const posts = this.socialMonitor.getRecentPosts(competitor.id);
            socialPosts.push(...posts.filter(p => p.postedAt >= periodStart && p.postedAt <= periodEnd));
        }
        // Generate report
     ...
     */
    determineSeverity(diffPercentage) {
        if (diffPercentage >= 50)
            return 'critical';
     ...
     */
    determinePricingSeverity(change) {
...
//# sourceMappingURL=CompetitorSentinel.js.map