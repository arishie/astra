...
export class CartTracker extends EventEmitter {
    ...
    constructor(abandonmentThreshold = 30) {
        super();
     ...
     */
    async trackCartUpdate(cartId, customerId, items, metadata) {
     ...
     */
    async markRecovered(cartId, purchaseValue) {
    ...
    async handleWebhook(event) {
        switch (event.type) {
            case 'cart.created':
            case 'cart.updated':
                await this.trackCartUpdate(event.data.cartId, event.data.customerId, event.data.items, {
...
export class RecoverySequencer {
     ...
     */
    async startSequence(cart, sequenceId = 'default') {
     ...
     */
    async executeStep(cart, step, sequence) {
     ...
     */
    getNextStep(cart, sequenceId = 'default') {
        const sequence = this.sequences.get(sequenceId);
        if (!sequence)
            return null;
        const completedSteps = cart.recoveryAttempts.length;
        if (completedSteps >= sequence.steps.length)
            return null;
     ...
     */
    getOptimalSendTime(scheduledTime, customerTimezone) {
        if (!customerTimezone)
...
export class VisualCartRenderer {
     ...
     */
    async renderCartWithDiscount(cart, discount) {
    ...
    calculateDiscountedTotal(cart, discount) {
...
export class DiscountEngine {
     ...
     */
    async calculateDiscount(cart, customerHistory, step) {
     ...
     */
    calculateDynamicDiscount(cart, customerHistory) {
     ...
     */
    async validateDiscount(code, cartValue, discount) {
...
Don't miss out - complete your order before they're gone!
     ...
     */
    build(templateId, cart, discount, extra) {
...
export class AnalyticsTracker extends EventEmitter {
     ...
     */
    trackRecovery(cart, purchaseValue, recoveringAttempt) {
...
export class AbandonedCartRecovery extends EventEmitter {
    ...
    setupEventHandlers() {
        this.cartTracker.on('cart:abandoned', async (cart) => {
     ...
     * Execute a recovery step
     */
    async executeRecoveryStep(cart, step) {
        ...
        // Build message
        const message = this.templateBuilder.build(step.messageTemplate, cart, discount || undefined);
        // Adapt for platform
     ...
     */
    async handleRecovery(cart, purchaseValue) {
     ...
     */
    getOrCreateCustomerHistory(customerId) {
     ...
     * Update customer history
     */
    updateCustomerHistory(customerId, history) {
...
//# sourceMappingURL=AbandonedCartRecovery.js.map