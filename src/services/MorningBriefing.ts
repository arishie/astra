...
export class DataAggregator {
    ...
    async aggregateData(
        ...
        if (sections.includes('calendar')) {
            fetchPromises.push(
                this.fetchCalendarEvents(userId, date).then((events) => {
        ...
        if (sections.includes('emails')) {
            fetchPromises.push(
                this.fetchEmailSummaries(userId).then((emails) => {
        ...
        if (sections.includes('news')) {
            fetchPromises.push(
                this.fetchNews(preferences.newsCategories).then((news) => {
                    results.news = news;
                })
            );
        }
        ...
        if (sections.includes('tasks')) {
            fetchPromises.push(
                this.fetchTasks(userId, date).then((tasks) => {
                    results.tasks = tasks;
                })
            );
        }
        ...
        if (sections.includes('weather')) {
            fetchPromises.push(
                this.fetchWeather(userId, preferences.timezone).then((weather) => {
        ...
        if (sections.includes('market')) {
            fetchPromises.push(
                this.fetchMarketData(preferences.marketSymbols).then((market) => {
        ...
        if (sections.includes('reminders')) {
            fetchPromises.push(
                this.fetchReminders(userId, date).then((reminders) => {
     ...
     */
    async fetchCalendarEvents(userId: string, date: Date): Promise<CalendarEvent[]> {
        const cacheKey = `calendar:${userId}:${date.toDateString()}`;
        const cached = this.getFromCache<CalendarEvent[]>(cacheKey);
    ...
    private async fetchFromCalendarAPI(userId: string, date: Date): Promise<CalendarEvent[]> {
     ...
     */
    async fetchEmailSummaries(userId: string, limit: number = 5): Promise<EmailSummary[]> {
        const cacheKey = `emails:${userId}`;
        const cached = this.getFromCache<EmailSummary[]>(cacheKey);
        if (cached) return cached;
    ...
    private async fetchFromEmailAPI(userId: string, limit: number): Promise<EmailSummary[]> {
     ...
     */
    async fetchNews(categories?: string[], limit: number = 5): Promise<NewsItem[]> {
    ...
    private async fetchFromNewsAPI(categories?: string[], limit?: number): Promise<NewsItem[]> {
     ...
     */
    async fetchTasks(userId: string, date: Date): Promise<TaskItem[]> {
        const cacheKey = `tasks:${userId}:${date.toDateString()}`;
        const cached = this.getFromCache<TaskItem[]>(cacheKey);
        if (cached) return cached;
    ...
    private async fetchFromTasksAPI(userId: string, date: Date): Promise<TaskItem[]> {
     ...
     */
    async fetchWeather(userId: string, timezone: string): Promise<WeatherData | null> {
    ...
    private async fetchFromWeatherAPI(userId: string, timezone: string): Promise<WeatherData | null> {
    ...
    private async fetchFromMarketAPI(symbols: string[]): Promise<MarketData[]> {
     ...
     */
    async fetchReminders(userId: string, date: Date): Promise<ReminderItem[]> {
        const cacheKey = `reminders:${userId}:${date.toDateString()}`;
        const cached = this.getFromCache<ReminderItem[]>(cacheKey);
        if (cached) return cached;

        try {
            const db = Database.getInstance();
            const result = await db.query<any>(
                `SELECT id, text, trigger_time, type, priority
                 FROM user_reminders
                 WHERE user_id = $1
                 AND trigger_time >= $2
                 AND trigger_time < $3
                 ORDER BY trigger_time ASC`,
                [userId, date, new Date(date.getTime() + 24 * 60 * 60 * 1000)]
            );

            const reminders: ReminderItem[] = result.rows.map((row) => ({
                id: row.id,
                text: row.text,
...
export class ScriptWriter {
    private wordsPerMinute: number = 150;
    private maxDurationMinutes: number = 5;

    constructor(options?: { wordsPerMinute?: number; maxDurationMinutes?: number }) {
    ...
    async createScript(
        ...
        const activeSections = sectionOrder.filter((s) =>
            preferences.sections.includes(s)
        );
        const wordsPerSection = Math.floor(targetWords / activeSections.length);

        for (const sectionType of sectionOrder) {
            if (!preferences.sections.includes(sectionType)) continue;
            ...
            const section = await this.generateSection(
                ...
                preferences,
                userName,
                Math.min(wordsPerSection, targetWords - totalWords)
            );
     ...
     */
    private generateGreeting(now: Date, timezone: string, userName?: string): string {
     ...
     */
    private generateCalendarSection(events: CalendarEvent[], wordBudget: number): string {
        if (events.length === 0) {
        ...
        for (let i = 0; i < maxEvents; i++) {
            ...
            if (this.countWords(parts.join(' ')) >= wordBudget) break;
        }
     ...
     */
    private generateWeatherSection(weather: WeatherData | null, wordBudget: number): string {
        if (!weather) {
        ...
        parts.push(
            `Currently it's ${Math.round(weather.currentTemp)} degrees with ${weather.condition.toLowerCase()}.`
        );
        parts.push(
            `Today's high will be ${Math.round(weather.highTemp)} and the low ${Math.round(weather.lowTemp)} degrees.`
        );
     ...
     */
    private generateNewsSection(news: NewsItem[], wordBudget: number): string {
        if (news.length === 0) {
     ...
     */
    private generateTasksSection(tasks: TaskItem[], wordBudget: number): string {
        if (tasks.length === 0) {
     ...
     */
    private generateMarketSection(market: MarketData[], wordBudget: number): string {
        if (market.length === 0) {
        ...
        for (const item of market) {
            ...
            parts.push(
                `${item.name || item.symbol} is ${direction} ${changeAbs}% at ${item.price.toFixed(2)}.`
            );
     ...
     */
    private generateEmailsSection(emails: EmailSummary[], wordBudget: number): string {
        if (emails.length === 0) {
     ...
     */
    private generateRemindersSection(reminders: ReminderItem[], wordBudget: number): string {
     ...
     */
    private generateClosing(userName?: string): string {
        const closings = [
    ...
    private trimToWordBudget(text: string, budget: number): string {
...
export class VoiceSynthesizer {
    private providers: Map<TTSProvider, TTSProviderClient> = new Map();
    ...
    async synthesize(text: string, config: VoiceConfig): Promise<Buffer> {
        ...
        if (!provider) {
            ...
            if (!fallbackProvider) {
                throw new Error(
                    `TTS provider '${config.provider}' not available and no fallback configured`
                );
            }
            console.warn(
                `[VoiceSynthesizer] Provider '${config.provider}' not available, using '${this.defaultProvider}'`
            );
     ...
     */
    async synthesizeScript(script: BriefingScript, config: VoiceConfig): Promise<Buffer> {
     ...
     */
    async listVoices(provider: TTSProvider): Promise<VoiceInfo[]> {
     ...
     */
    private generateSilence(durationSeconds: number): Buffer {
...
class ElevenLabsClient implements TTSProviderClient {
    ...
    async listVoices(): Promise<VoiceInfo[]> {
        ...
        return data.voices.map((voice) => ({
            id: voice.voice_id,
            name: voice.name,
            gender: voice.labels?.gender as VoiceGender,
...
// OpenAI TTS Client
class OpenAITTSClient implements TTSProviderClient {
    private apiKey: string;
...
// Google TTS Client
class GoogleTTSClient implements TTSProviderClient {
    private apiKey: string;
    ...
    async listVoices(): Promise<VoiceInfo[]> {
        ...
        return data.voices.map((voice) => ({
            id: voice.name,
            name: voice.name,
            gender: voice.ssmlGender.toLowerCase() as VoiceGender,
...
export class AudioMixer {
    private introMusicPath?: string;
    private outroMusicPath?: string;
    private transitionSoundPath?: string;
    private backgroundMusicPath?: string;
    ...
    async mixAudio(
        ...
        if (options.includeIntro && this.introMusicPath) {
            try {
                const introBuffer = await this.loadAndProcessMusic(
                    this.introMusicPath,
                    options.introDuration || 3
                );
        ...
        if (options.includeOutro && this.outroMusicPath) {
            try {
                const outroBuffer = await this.loadAndProcessMusic(
                    this.outroMusicPath,
                    options.outroDuration || 3
                );
     ...
     */
    private async loadAndProcessMusic(
        filePath: string,
     ...
     */
    async normalizeAudio(buffer: Buffer, targetDb: number = -16): Promise<Buffer> {
    ...
    setMusicPaths(paths: {
        ...
        if (paths.intro) this.introMusicPath = paths.intro;
        if (paths.outro) this.outroMusicPath = paths.outro;
        if (paths.transition) this.transitionSoundPath = paths.transition;
        if (paths.background) this.backgroundMusicPath = paths.background;
    }
}
...
export class PodcastPublisher {
     ...
     */
    async getBriefing(userId: string, briefingId: string): Promise<Buffer | null> {
        const filePath = path.join(
            this.storageBasePath,
            userId,
            `briefing-${briefingId}.mp3`
        );
     ...
     */
    async listBriefings(userId: string): Promise<BriefingMetadata[]> {
        const userDir = path.join(this.storageBasePath, userId);
        ...
        try {
            ...
            const briefings: BriefingMetadata[] = [];
     ...
     * Deletes a briefing.
     */
    async deleteBriefing(userId: string, briefingId: string): Promise<boolean> {
        const userDir = path.join(this.storageBasePath, userId);
...
export class ScheduleManager extends EventEmitter {
    ...
    async setSchedule(config: ScheduleConfig): Promise<void> {
        ...
        try {
            await this.db.query(
                `INSERT INTO briefing_schedules (user_id, enabled, delivery_time, timezone, enabled_days)
                 VALUES ($1, $2, $3, $4, $5)
                 ON CONFLICT (user_id)
                 DO UPDATE SET
    ...
    async removeSchedule(userId: string): Promise<boolean> {
        ...
        try {
            await this.db.query(
                'DELETE FROM briefing_schedules WHERE user_id = $1',
                [userId]
            );
        } catch (error) {
     ...
     */
    private async loadSchedulesFromDB(): Promise<void> {
        try {
            const result = await this.db.query<any>(
                'SELECT * FROM briefing_schedules WHERE enabled = true'
            );
     ...
     */
    private scheduleJob(config: ScheduleConfig): void {
     ...
     */
    private async executeScheduledJob(config: ScheduleConfig): Promise<void> {
        ...
        try {
            await this.db.query(
                'UPDATE briefing_schedules SET last_run = NOW() WHERE user_id = $1',
                [config.userId]
            );
        } catch (error) {
...
export class MorningBriefingService extends EventEmitter {
    ...
    private podcastPublisher: PodcastPublisher;
    private scheduleManager: ScheduleManager;
    private db: Database;
    private userPreferencesCache: Map<string, BriefingPreferences> = new Map();
    ...
    constructor(options?: {
        ...
        this.scheduleManager.on('briefing-due', async (userId: string) => {
            try {
     ...
     */
    async generateBriefing(userId: string): Promise<GeneratedBriefing> {
        ...
        try {
            ...
            const script = await this.scriptWriter.createScript(
                aggregatedData,
                preferences,
                userName
            );
            ...
            const speechBuffer = await this.voiceSynthesizer.synthesizeScript(
                script,
                preferences.voice
            );
            ...
            const publishResult = await this.podcastPublisher.publish(
                briefingId,
                userId,
                finalAudio,
                {
            ...
            console.log(
                `[MorningBriefingService] Briefing ${briefingId} completed in ${duration.toFixed(2)}s`
            );
            ...
            const failedResult: GeneratedBriefing = {
                id: briefingId,
                userId,
    ...
    async setUserPreferences(
        ...
        const updated: BriefingPreferences = {
            ...current,
            ...preferences,
            voice: {
                ...current.voice,
        ...
        // Save to database
        await this.db.query(
            `INSERT INTO briefing_preferences (user_id, preferences)
             VALUES ($1, $2)
             ON CONFLICT (user_id)
     ...
     */
    async getUserPreferences(userId: string): Promise<BriefingPreferences> {
        // Check cache first
        ...
        try {
            const result = await this.db.query<any>(
                'SELECT preferences FROM briefing_preferences WHERE user_id = $1',
                [userId]
            );
    ...
    async scheduleDaily(
        ...
        console.log(
            `[MorningBriefingService] Scheduled daily briefing for user ${userId} at ${deliveryTime} ${timezone}`
        );
    }
     ...
     */
    async cancelSchedule(userId: string): Promise<void> {
     ...
     */
    async getBriefing(userId: string, briefingId: string): Promise<GeneratedBriefing | null> {
        try {
            const result = await this.db.query<any>(
     ...
     */
    async getAvailableVoices(provider?: TTSProvider): Promise<Map<TTSProvider, VoiceInfo[]>> {
        const result = new Map<TTSProvider, VoiceInfo[]>();
     ...
     */
    async getBriefingAudio(userId: string, briefingId: string): Promise<Buffer | null> {
    ...
    async generatePodcastFeed(
        userId: string,
        options?: { title?: string; description?: string; author?: string; imageUrl?: string }
    ): Promise<string> {
     ...
     */
    async cleanupOldBriefings(userId: string, retentionDays: number = 30): Promise<number> {
     ...
     */
    private async saveBriefingRecord(briefing: GeneratedBriefing): Promise<void> {
        try {
            await this.db.query(
                `INSERT INTO briefings (id, user_id, script, audio_url, audio_path, audio_duration, status, preferences, aggregated_data, generated_at, published_at)
...
let morningBriefingServiceInstance: MorningBriefingService | null = null;
