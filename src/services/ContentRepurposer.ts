...
export interface RepurposedContent {
    ...
    warnings: string[];
    thread?: RepurposedContent[]; // For platforms supporting threads
    metadata: {
...
const PLATFORM_CONSTRAINTS: Record<ContentPlatform, PlatformConstraints> = {
    twitter: {
        maxTextLength: 280,
        maxHashtags: 5,
        maxMentions: 10,
...
export class ToneAdjuster {
    private tonePatterns: Map<ContentTone, TonePattern>;
    ...
    private detectTone(text: string): ContentTone {
        const scores: Map<ContentTone, number> = new Map();
        ...
        for (const [tone, pattern] of this.tonePatterns) {
            ...
            if (pattern.emojiUsage === 'none' && emojiCount === 0) score += 2;
            if (pattern.emojiUsage === 'minimal' && emojiDensity < 1) score += 1;
            if (pattern.emojiUsage === 'moderate' && emojiDensity >= 1 && emojiDensity < 3) score += 1;
            if (pattern.emojiUsage === 'frequent' && emojiDensity >= 3) score += 2;

            scores.set(tone, score);
        }

        let maxTone: ContentTone = 'conversational';
    ...
    private findReplacement(word: string, preferredWords: string[]): string | null {
        ...
        for (const [, words] of Object.entries(wordClasses)) {
            if (words.includes(word.toLowerCase())) {
                const available = preferredWords.filter(w =>
                    words.includes(w.toLowerCase()) || wordClasses.positive.includes(w.toLowerCase())
                );
    ...
    private adjustEmojis(text: string, usage: 'none' | 'minimal' | 'moderate' | 'frequent'): string {
    ...
    private calculateToneConfidence(text: string, targetTone: ContentTone): number {
...
export class HashtagGenerator {
    private trendingHashtags: Map<ContentPlatform, Map<string, TrendingHashtag>>;
    private industryHashtags: Map<string, string[]>;
    ...
    private initializeIndustryHashtags(): void {
    ...
    ): Promise<HashtagGeneratorResult> {
    ...
    private extractKeywords(content: string): Array<{ word: string; score: number }> {
    ...
    private detectIndustries(content: string): string[] {
    ...
    private formatHashtag(word: string): string {
    ...
    private isValidHashtag(hashtag: string, platform: ContentPlatform): boolean {
        // Basic validation
        if (hashtag.length < 2 || hashtag.length > 100) return false;
        if (!/^#[a-zA-Z0-9_]+$/.test(hashtag)) return false;
        ...
        if (platform === 'linkedin') {
            ...
            if (casualTags.includes(tagLower)) return false;
        }

        return true;
    }

    private getPlatformSpecificHashtags(platform: ContentPlatform): string[] {
        const platformTags: Record<ContentPlatform, string[]> = {
    ...
    getTrending(platform: ContentPlatform, limit: number = 10): TrendingHashtag[] {
        const platformTrending = this.trendingHashtags.get(platform);
        if (!platformTrending) return [];
...
export class MediaTransformer {
    private transformationQueue: Map<string, MediaTransformationJob>;
    ...
    async transform(
        ...
        if (media.type === 'image' || media.type === 'video') {
            ...
            const aspectRatioTransform = this.calculateAspectRatioTransform(
                media,
                constraints.mediaAspectRatios,
                options.resizeStrategy || 'fit'
            );
    ...
    async transformBatch(
        ...
        const results = await Promise.all(
            limitedList.map(media => this.transform(media, targetPlatform, options))
        );
    ...
    private calculateAspectRatioTransform(
        media: MediaAsset,
        targetRatios: AspectRatio[],
        strategy: 'fit' | 'fill' | 'crop'
    ...
    getMaxDimensions(
        platform: ContentPlatform
    ): { maxWidth: number; maxHeight: number; aspectRatios: AspectRatio[] } {
        const constraints = PLATFORM_CONSTRAINTS[platform];

        // Standard max dimensions per platform
        const maxDimensions: Record<ContentPlatform, { maxWidth: number; maxHeight: number }> = {
...
export class ContentScheduler {
    private optimalTimes: Map<ContentPlatform, ScheduleSlot[]>;
    private scheduledContent: Map<string, ScheduledContentItem>;
    ...
    getOptimalTime(
        ...
        const sortedSlots = [...platformSlots].sort(
        ...
        for (const slot of sortedSlots) {
            ...
            if (options.notBefore && targetDate < options.notBefore) continue;
            if (options.notAfter && targetDate > options.notAfter) continue;
    ...
    getOptimalTimesForBatch(
        ...
        fromDate: Date = new Date(),
        options: ScheduleOptions = {}
    ): Map<ContentPlatform, Date> {
        const result = new Map<ContentPlatform, Date>();
    ...
    schedule(
        ...
        const item: ScheduledContentItem = {
    ...
    getScheduledContent(
        filter?: { platform?: ContentPlatform; status?: ScheduledContentStatus }
    ): ScheduledContentItem[] {
    ...
    getUpcoming(limit: number = 10): ScheduledContentItem[] {
        const now = new Date();
...
abstract class BasePlatformAdapter implements PlatformAdapter {
    abstract platform: ContentPlatform;
    abstract constraints: PlatformConstraints;

    protected toneAdjuster: ToneAdjuster;
    protected hashtagGenerator: HashtagGenerator;
    ...
    abstract adapt(content: ContentInput, options: RepurposeOptions): Promise<RepurposedContent>;

    protected truncateText(text: string, maxLength: number, preserveWords: boolean = true): string {
    ...
    protected extractLinks(text: string): string[] {
        const urlRegex = /https?:\/\/[^\s]+/g;
        return text.match(urlRegex) || [];
    }

    protected removeExcessiveWhitespace(text: string): string {
...
class TwitterAdapter extends BasePlatformAdapter {
    ...
    async adapt(content: ContentInput, options: RepurposeOptions): Promise<RepurposedContent> {
        const result = this.createBaseResult(content, options) as RepurposedContent;
        const warnings: string[] = [];

        // Adjust tone
        const toneResult = this.toneAdjuster.adjustTone(
            content.text,
            options.targetTone || 'conversational',
            this.platform
        );
        ...
        if (content.media && content.media.length > 0) {
            result.media = await this.mediaTransformer.transformBatch(
                content.media,
                this.platform,
...
class LinkedInAdapter extends BasePlatformAdapter {
    ...
    private structureForLinkedIn(text: string): string {
        const sentences = text.match(/[^.!?]+[.!?]+/g) || [text];
        ...
        const hasCallToAction = ctaKeywords.some(keyword =>
            formattedText.toLowerCase().includes(keyword)
        );
...
class TikTokAdapter extends BasePlatformAdapter {
    ...
    async adapt(content: ContentInput, options: RepurposeOptions): Promise<RepurposedContent> {
        ...
        if (content.media && content.media.length > 0) {
            ...
            if (videoMedia.length > 0) {
                result.media = await this.mediaTransformer.transformBatch(
                    [videoMedia[0]], // TikTok only supports one video
                    this.platform,
                    {
                        ...options.mediaTransformations,
                        resizeStrategy: 'fill',
    ...
    private convertToScript(text: string): string {
...
class YouTubeAdapter extends BasePlatformAdapter {
    ...
    private formatDescription(text: string, content: ContentInput): string {
...
class BlogAdapter extends BasePlatformAdapter {
    ...
    private formatBlogPost(text: string, content: ContentInput): string {
...
class EmailAdapter extends BasePlatformAdapter {
    ...
    async adapt(content: ContentInput, options: RepurposeOptions): Promise<RepurposedContent> {
        ...
        if (content.media && content.media.length > 0) {
            ...
            result.media = await this.mediaTransformer.transformBatch(
                imageMedia.slice(0, this.constraints.maxMediaCount),
                this.platform,
                {
                    ...options.mediaTransformations,
    ...
    private formatNewsletterEmail(text: string, content: ContentInput): string {
        const sections: string[] = [];
    ...
    private generateSubjectLine(text: string): string {
...
export class ContentRepurposer extends EventEmitter {
    private adapters: Map<ContentPlatform, PlatformAdapter>;
    ...
    private initializeAdapters(): void {
        ...
        this.adapters.set('blog', new BlogAdapter(
            this.toneAdjuster,
            this.hashtagGenerator,
            this.mediaTransformer
        ));
    ...
    async repurpose(
        ...
        const repurposedContent = new Map<ContentPlatform, RepurposedContent>();
        const errors = new Map<ContentPlatform, string>();
        ...
        if (!content.text || content.text.trim().length === 0) {
            return {
                ...
                targetPlatforms,
                repurposedContent,
                errors: new Map([['all' as ContentPlatform, 'Content text is required']]),
    ...
    async repurposeToAll(
        content: ContentInput,
        sourcePlatform: ContentPlatform,
        options: RepurposeOptions = {}
    ): Promise<RepurposeResult> {
        const allPlatforms = Array.from(this.adapters.keys()).filter(
            p => p !== sourcePlatform
        );
    ...
    private updatePlatformStats(platform: ContentPlatform, success: boolean): void {
    ...
    async scheduleContent(
        ...
        targetPlatforms: ContentPlatform[],
        options: RepurposeOptions = {}
    ): Promise<Map<ContentPlatform, ScheduledContentItem>> {
        const result = await this.repurpose(content, sourcePlatform, targetPlatforms, {
            ...options,
            scheduleOptimal: true,
        });

        const scheduledItems = new Map<ContentPlatform, ScheduledContentItem>();
    ...
    validateContent(
        ...
        if (content.text.length > constraints.maxTextLength) {
            errors.push(
                `Text exceeds maximum length (${content.text.length}/${constraints.maxTextLength})`
            );
        }
        ...
        if (hashtags.length > constraints.maxHashtags) {
            warnings.push(
                `Too many hashtags (${hashtags.length}/${constraints.maxHashtags})`
            );
        }
        ...
        if (content.media) {
            if (content.media.length > constraints.maxMediaCount) {
                warnings.push(
                    `Too many media items (${content.media.length}/${constraints.maxMediaCount})`
                );
            }
            ...
            for (const media of content.media) {
                ...
                if (media.type === 'video' && media.duration && constraints.maxVideoLength) {
                    if (media.duration > constraints.maxVideoLength) {
                        warnings.push(
                            `Video exceeds maximum duration (${media.duration}s/${constraints.maxVideoLength}s)`
                        );
                    }
                }
                ...
                if (media.type === 'image' && media.fileSize && constraints.maxImageSize) {
                    if (media.fileSize > constraints.maxImageSize) {
                        warnings.push(
                            `Image exceeds maximum size (${Math.round(media.fileSize / 1024 / 1024)}MB/${Math.round(constraints.maxImageSize / 1024 / 1024)}MB)`
                        );
                    }
                }
            }
...
let contentRepurposerInstance: ContentRepurposer | null = null;

export function getContentRepurposer(options?: ContentRepurposerOptions): ContentRepurposer {
...
export function createContentRepurposer(options?: ContentRepurposerOptions): ContentRepurposer {
...
export class ContentRepurposerBridge {
    private repurposer: ContentRepurposer;
    ...
    async handleIncomingMessage(
        ...
        content: string,
        metadata?: Record<string, any>
    ): Promise<Map<ContentPlatform, RepurposedContent>> {
        const contentInput: ContentInput = {
            text: content,
            metadata,
        };

        const targetPlatforms = this.repurposer.getSupportedPlatforms().filter(
            p => p !== platform
        );

        const result = await this.repurposer.repurpose(
            contentInput,
            platform,
            targetPlatforms
        );
    ...
    async crossPost(
        ...
        return this.repurposer.repurpose(
            content,
            sourcePlatform,
            targetPlatforms,
            options
