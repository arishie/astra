...
export class DiscordBridge extends AbstractBridge {
    ...
    async sendEmbed(
        ...
        try {
            ...
            if (embed.fields) discordEmbed.addFields(embed.fields);
    ...
    private getAttachmentType(message: Message): 'image' | 'video' | 'audio' | 'document' | undefined {
    ...
    private isTextChannel(channel: any): channel is TextChannel | DMChannel {
        return channel && typeof channel.send === 'function';
    }

    getBotInfo(): { username?: string; id?: string } {
        return {
...
export function createDiscordBridgeFactory() {
    return async (config: BridgeConfig): Promise<DiscordBridge> => {
