...
import { JWTService, type TokenPayload, type TokenPair } from '../auth/JWTService.js';
import { OAuthProvider, type OAuthUserData } from '../auth/OAuthProvider.js';
...
export class Gateway {
    ...
    private elevationRequests: Map<string, ElevationRequest> = new Map();
    private elevationCallbacks: Map<string, (approved: boolean) => void> = new Map();
    private userServiceCallback?: (oauthData: OAuthUserData) => Promise<{ userId: string; email: string } | null>;
    ...
    setUserServiceCallback(callback: (oauthData: OAuthUserData) => Promise<{ userId: string; email: string } | null>) {
    ...
    generateOAuthUrl(provider: string): { url: string; state: string } | null {
    ...
    async authenticateWithToken(accessToken: string): Promise<UserSession | null> {
    ...
    async requestElevation(
        accessToken: string,
        reason: string
    ): Promise<{ elevationId: string; requiresApproval: boolean } | null> {
    ...
    async approveElevation(elevationId: string, approved: boolean): Promise<boolean> {
    ...
    async waitForElevation(elevationId: string, timeoutMs: number = 60000): Promise<boolean> {
        const request = this.elevationRequests.get(elevationId);
        if (!request) {
            return false;
        }
    ...
    async elevateToken(accessToken: string): Promise<string | null> {
        return this.jwtService.elevateToken(accessToken, AuthLevel.HIGH_SECURITY);
    }

    async checkAccess(accessToken: string, requiredLevel: AuthLevel): Promise<boolean> {
        const session = await this.authenticateWithToken(accessToken);
        if (!session) {
            return false;
        }
    ...
    getPendingElevations(): Array<{ id: string; request: ElevationRequest }> {
        const pending: Array<{ id: string; request: ElevationRequest }> = [];
        const now = Date.now();
    ...
    generateSystemToken(systemId: string): Promise<TokenPair> {
        ...
        return this.jwtService.generateTokenPair(
            `system:${systemId}`,
            `${systemId}@system.astra.ai`,
            AuthLevel.HIGH_SECURITY
        );
    }

    async verifySystemToken(accessToken: string, expectedSystemId: string): Promise<boolean> {
        const session = await this.authenticateWithToken(accessToken);
        if (!session) {
            return false;
        }
        ...
        return (
            session.userId === `system:${expectedSystemId}` &&
