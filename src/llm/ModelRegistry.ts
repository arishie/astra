...
export class ModelRegistry {
    ...
    private deriveEncryptionKey(userId: string): Buffer {
    ...
    private decrypt(encrypted: EncryptedConfig): string {
    ...
    public getConfig(name: string): ModelConfig | undefined {
        ...
        if (config.apiKey && config.apiKey.startsWith('encrypted:')) {
            try {
                const encryptedData = JSON.parse(config.apiKey.substring(10)) as EncryptedConfig;
    ...
    public getConfigMasked(name: string): Omit<ModelConfig, 'apiKey'> & { apiKeyMasked: string } | undefined {
        const config = this.configs.get(name);
        if (!config) return undefined;
    ...
    public updateApiKey(name: string, newApiKey: string): boolean {
        const config = this.configs.get(name);
    ...
    public async validateApiKey(name: string): Promise<{ valid: boolean; error?: string }> {
    ...
    private async validateOpenAIKey(config: ModelConfig): Promise<{ valid: boolean; error?: string }> {
        try {
    ...
    private async validateAnthropicKey(config: ModelConfig): Promise<{ valid: boolean; error?: string }> {
        try {
    ...
    private async validateGoogleKey(config: ModelConfig): Promise<{ valid: boolean; error?: string }> {
        try {
    ...
    private async validateOpenAICompatibleKey(config: ModelConfig): Promise<{ valid: boolean; error?: string }> {
        if (!config.baseUrl) {
    ...
    public exportForUser(): Array<Omit<ModelConfig, 'apiKey'> & { hasApiKey: boolean }> {
        const result: Array<Omit<ModelConfig, 'apiKey'> & { hasApiKey: boolean }> = [];
