...
export class SmartCommands extends EventEmitter {
    ...
    private docs: GoogleDocsIntegration;
    private voiceProcessor: VoiceProcessor;
    private screenshotAI: ScreenshotUnderstanding;
     ...
     */
    public async processMessage(message: IncomingMessage): Promise<CommandResponse> {
     ...
     */
    private async handleTextMessage(message: IncomingMessage, context: UserContext): Promise<CommandResponse> {
     ...
     */
    private async handleVoiceMessage(message: IncomingMessage, context: UserContext): Promise<CommandResponse> {
        ...
        const result = await this.voiceProcessor.processVoiceMessage({
            ...
            platform: message.platform as 'whatsapp' | 'telegram' | 'discord',
     ...
     */
    private async handleImageMessage(message: IncomingMessage, context: UserContext): Promise<CommandResponse> {
        if (!message.mediaPath) {
     ...
     */
    private async handleDocumentMessage(message: IncomingMessage, context: UserContext): Promise<CommandResponse> {
        if (!message.mediaPath) {
     ...
     */
    private handleConnectCommand(text: string, userId: string): CommandResponse {
     ...
     */
    private async handleDisconnectCommand(text: string, userId: string): Promise<CommandResponse> {
        const provider = text.match(/(google|notion|outlook|github)/i)?.[1].toLowerCase();
        if (!provider) {
            return { text: 'Please specify which service to disconnect.' };
        }

        await this.integrationManager.removeIntegration(userId, provider as any);
     ...
     */
    private async handleCalendarCommand(text: string, userId: string, context: UserContext): Promise<CommandResponse> {
        ...
        if (text.match(/what(?:'s| is).*(?:calendar|schedule|today|tomorrow)/i)) {
            try {
                ...
                const eventList = events.map(e =>
                    `• ${e.start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - ${e.title}${e.location ? ` (${e.location})` : ''}`
                ).join('\n');
        ...
        if (text.match(/free|available|slot/i)) {
            try {
                ...
                const slotList = slots.slice(0, 3).map(s =>
                    `• ${s.start.toLocaleDateString()} at ${s.start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
                ).join('\n');
                return {
     ...
     */
    private async handleEmailCommand(text: string, userId: string, context: UserContext): Promise<CommandResponse> {
        const connected = this.integrationManager.getConnectedIntegrations(userId);
        if (!connected.includes('google') && !connected.includes('outlook')) {
            return {
     ...
     */
    private handleReminderCommand(text: string, userId: string, context: UserContext): CommandResponse {
     ...
     */
    private async handleNoteCommand(text: string, userId: string, context: UserContext): Promise<CommandResponse> {
        const connected = this.integrationManager.getConnectedIntegrations(userId);
     ...
     */
    private async handleSearchCommand(text: string, userId: string, context: UserContext): Promise<CommandResponse> {
     ...
     */
    private async handleConversation(text: string, userId: string, context: UserContext): Promise<CommandResponse> {
     ...
     */
    private async handlePendingAction(text: string, context: UserContext): Promise<CommandResponse> {
        ...
        switch (action.type) {
            ...
            case 'screenshot_action':
                ...
                const matchingAction = action.data.actions.find((a: any) =>
     ...
     * Get help message
     */
    private getHelpMessage(userId: string): CommandResponse {
        const connected = this.integrationManager.getConnectedIntegrations(userId);
...
What would you like to do?`,
     ...
     * Get status message
     */
    private getStatusMessage(userId: string): CommandResponse {
        const connected = this.integrationManager.getConnectedIntegrations(userId);
     ...
     */
    private getOrCreateContext(userId: string, platform: string): UserContext {
